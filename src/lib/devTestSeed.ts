...
// 지우세요. 개발 중 "맛있는 하루"/"다가올 한 입" 탭에 테스트 데이터를 빠르게
// 채워 넣기 위한 임시 유틸이에요. 실제 서비스 데이터가 아니라 기기 로컬 저장
// (localStorage)만 건드리고, 서버(Supabase 등)에는 아무 영향이 없어요.
...
// 새로고침해요. 확인 다이얼로그는 화면(ProfileView.tsx)에서 먼저 띄우고, 사용자가
// 계속하기를 눌렀을 때만 이 함수를 호출해요.
export function applyDevTestSeed(): void {
