...
// user_hash에 해당하는 프로필(닉네임)을 저장해요. profiles 테이블은 user_hash가
// 고유(primary key 또는 unique)하다고 가정하고 upsert해요.
export async function saveProfile(
  ...
  try {
    const { error } = await supabase
      .from("anon_profiles")
      .upsert({ user_hash: userHash, nickname }, { onConflict: "user_hash" });
